...
fly.interceptors.request.use(
  request => {
    fly.lock();
    return authStore.checkToken(request).then(() => {
      request.headers['AuthToken'] = wx.getStorageSync(AUTH_TOKEN_KEY);
      request.headers['Content-Type'] = request.method === 'POST' ? 'application/x-www-form-urlencoded' : 'application/json';
      return request;
    }).finally(() => {
      fly.unlock();
    });
  ...
  response => {
    ...
    if (data.code !== 200) {
      ...
      if (data.code === 3001 && retryTimes--) {
        fly.lock();
        return authStore.login().finally(() => {
          fly.unlock();
        }).then(() => {
          // 重新发起请求
